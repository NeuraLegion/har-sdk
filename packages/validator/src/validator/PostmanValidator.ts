...
export class PostmanValidator implements Validator {
  ...
  public async verify(spec: Postman.Document): Promise<void> {
    ok(spec, 'Postman collection is not provided.');
    ok(spec.info, '"info" section is missed in the collection.');

    this.validateVersion(spec);

    const schemaId: string = spec.info.schema
      ? parse(spec.info.schema).dir + '/'
      : '';
  ...
  private validateVersion(spec: Postman.Document): void {
    ok(spec, 'Postman collection is not provided.');
    ok(spec.info, '"info" section is missed in the collection.');

    if (spec.info.version) {
      const { version: versionObject } = spec.info;
